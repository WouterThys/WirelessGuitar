...
void InitInterrupts(void) {
    ...
    /* T I M E R 1 */
    _T1IP = TMR1_IP;// Set Timer 1 Interrupt Priority Level Highest
    ...
    _DMA0IP = DMA0_IP;
    _DMA0IE = 0;    // Set the interrupt enable bit
...
uint16_t pwm1[NUMSAMP], pwm2[NUMSAMP];
uint16_t pwm_switch;
...
void __attribute__((__interrupt__, no_auto_psv)) _T1Interrupt(void) {
    _T1IF = 0;      // Clear Timer3 interrupt flag
}
...
//        _LATA1 = !_LATA1;
//     if(dmaBuffer == 0) {
//         payload[PL_MSB] = ((AdcBufferA[0] & 0x0300)>>8);
//         payload[PL_LSB] = (AdcBufferA[0] & 0x00FF);
//         for(i=0; i<(PL_LENGTH-PL_DIFF); i++) {
//             payload[PL_DIFF+i] = AdcBufferA[i]-AdcBufferA[i+1];
//         }
//     } else {
//         payload[PL_MSB] = ((AdcBufferB[0] & 0x0300)>>8);
//         payload[PL_LSB] = (AdcBufferB[0] & 0x00FF);
//         for(i=0; i<(PL_LENGTH-PL_DIFF); i++) {
//             payload[PL_DIFF+i] = AdcBufferB[i]-AdcBufferB[i+1];
//         }
//     }
//    dmaBuffer ^= 1;
