...
void InitInterrupts(void) {
    ...
    /* I N T 1 */           //--> interrupt from the nRF
    ...
    /* T I M E R 3 */       //--> for placing received data on pwm @ sample frequency
    _T3IP = TMR3_IP; // Set Timer 3 Interrupt Priority Level
    _T3IF = 0; // Clear Timer 3 Interrupt Flag
    _T3IE = 1; // Enable Timer3 interrupt

    /* U A R T 1 */         // --> Talking with Anouchka
    _U1RXIF = 0;
    _U1RXIP = U1RX_IP;
    _U1RXIE = 1;

    /* T I M E R 4 */       //--> for metronome
   _T4IF = 0;
   _T4IP = TMR4_IP;
   _T4IE = 1;
...
//        _LATB10 = 0;
    ...
    if (nrf_int_status & (1 << RX_DR)) { // / IRQ: Package has been received
        ...
        dmaBuffer ^= 1; // Switch buffers, no coallision possible
//        _LATB12 = 1;
    } else {
//        _LATB12 = 0;
    }
...
uint16_t uart_block_count;
uint16_t U2SoftFlag;
struct SerialStruct SerialRx;
void __attribute__((__interrupt__, no_auto_psv)) _U1RXInterrupt(void) {
        U2Buffer[uart_block_count] = U1RXREG; // Read data
        uart_block_count++;
        if (uart_block_count == UART_LENGTH) {
            uart_block_count = 0;
            SerialRx.TransmitAddress = (U2Buffer[SER_COMMAND] & 0x00F0);
            SerialRx.Command = (U2Buffer[SER_COMMAND] & 0x000F);
            SerialRx.Hardware = U2Buffer[SER_HW];
            SerialRx.Dat1 = U2Buffer[SER_DAT1];
            SerialRx.Dat2 = U2Buffer[SER_DAT2];
            SerialRx.Dat3 = U2Buffer[SER_DAT3];
            SerialRx.Dat4 = U2Buffer[SER_DAT4];
            SerialRx.Dat5 = U2Buffer[SER_DAT5];
            SerialRx.Dat6 = U2Buffer[SER_DAT6];
            U2SoftFlag = 1;
        }
