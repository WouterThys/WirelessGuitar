...
uint8_t payload1[PL_LENGTH]; //holds the payload
uint8_t payload2[PL_LENGTH]; //holds the payload
uint16_t PwmVal[NUMSAMP];   // Pwm values
...
void __attribute__((__interrupt__, no_auto_psv)) _DMA0Interrupt(void) {
    ...
    if (dmaBuffer == 0) {
        for (i = 0; i < PL_BLOCKS; i++) {
            // MSBs of block
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] = (AdcBufferA[4 * i ] & MSB_MASK) >> 2; // MSB1
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] += (AdcBufferA[4 * i + 1] & MSB_MASK) >> 4; // MSB2
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] += (AdcBufferA[4 * i + 2] & MSB_MASK) >> 6; // MSB3
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] += (AdcBufferA[4 * i + 3] & MSB_MASK) >> 8; // MSB4

            // LSBs of block
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 1] = (AdcBufferA[4 * i ] & LSB_MASK); // LSB1
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 2] = (AdcBufferA[4 * i + 1] & LSB_MASK); // LSB2
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 3] = (AdcBufferA[4 * i + 2] & LSB_MASK); // LSB3
            payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 4] = (AdcBufferA[4 * i + 3] & LSB_MASK); // LSB4
        }

        uint16_t i;
        // payload1[0] = command
        for (i = 0; i < PL_BLOCKS; i++) {
            PwmVal1[4 * i ] = (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b11000000) << 2; // MSB1
            PwmVal1[4 * i + 1] = (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b00110000) << 4; // MSB2
            PwmVal1[4 * i + 2] = (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b00001100) << 6; // MSB3
            PwmVal1[4 * i + 3] = (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b00000011) << 8; // MSB4

            PwmVal1[4 * i ] += (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 1]); // LSB1
            PwmVal1[4 * i + 1] += (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 2]); // LSB1
            PwmVal1[4 * i + 2] += (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 3]); // LSB1
            PwmVal1[4 * i + 3] += (payload1[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 4]); // LSB1
        }

    } else { //T3CONbits.TON = 0; AD1CON1bits.ADON = 0;    // stop after filling 2 buffers (debugging only)
        for (i = 0; i < PL_BLOCKS; i++) {
            // MSBs of block
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] = (AdcBufferB[4 * i ] & MSB_MASK) >> 2; // MSB1
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] += (AdcBufferB[4 * i + 1] & MSB_MASK) >> 4; // MSB2
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] += (AdcBufferB[4 * i + 2] & MSB_MASK) >> 6; // MSB3
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] += (AdcBufferB[4 * i + 3] & MSB_MASK) >> 8; // MSB4

            // LSBs of block
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 1] = (AdcBufferB[4 * i ] & LSB_MASK); // LSB1
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 2] = (AdcBufferB[4 * i + 1] & LSB_MASK); // LSB2
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 3] = (AdcBufferB[4 * i + 2] & LSB_MASK); // LSB3
            payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 4] = (AdcBufferB[4 * i + 3] & LSB_MASK); // LSB4
        }

        // payload2[0] = command
        for (i = 0; i < PL_BLOCKS; i++) {
            PwmVal2[4 * i ] = (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b11000000) << 2; // MSB1
            PwmVal2[4 * i + 1] = (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b00110000) << 4; // MSB2
            PwmVal2[4 * i + 2] = (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b00001100) << 6; // MSB3
            PwmVal2[4 * i + 3] = (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i)] & 0b00000011) << 8; // MSB4

            PwmVal2[4 * i ] += (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 1]); // LSB1
            PwmVal2[4 * i + 1] += (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 2]); // LSB1
            PwmVal2[4 * i + 2] += (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 3]); // LSB1
            PwmVal2[4 * i + 3] += (payload2[PL_BLOCK_START + (PL_BLOCK_LENGTH * i) + 4]); // LSB1
        }
